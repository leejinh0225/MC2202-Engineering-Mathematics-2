def render_home(shell, lecture_card, count=1):
...
<section class="home-reading" aria-labelledby="reading-title"><p class="section-kicker">Reading modes</p><h2 class="section-title" id="reading-title">익숙한 설명으로 읽고, 막히면 기초부터.</h2><p class="section-lead">상단의 뉴비 모드 버튼으로 전환합니다. 선택한 모드는 정리노트와 자료 목록에도 이어집니다.</p><div class="grid-2"><div class="card"><h3>기본 모드</h3><p>밝은 화면에서 핵심 개념, 공식의 유도, 단계별 문제 풀이를 이어 읽습니다. 원본 페이지와 해설을 함께 보며 흐름을 따라갈 수 있습니다.</p></div><div class="card"><h3>뉴비 모드</h3><p>다크 화면에서 본문 자체를 기초부터 풀어 쓴 해설로 바꿉니다. 왜 이 개념이 필요한지, 왜 다음 계산을 하는지, 결과가 무엇을 뜻하는지 연결합니다. English(한국어 번역) 용어 표기는 유지합니다.</p></div></div></section>
<aside class="source-note"><strong>자료와 해설 안내</strong><p>강의 영상·스크립트 없이 원본 PDF를 바탕으로 작성했습니다. 보충 설명과 문제 풀이는 편집자 해설이며, 원본의 오류와 조건 누락은 각 노트의 교정 기록에서 구분합니다.</p></aside>'''
    ...
    return page.replace('원본 PDF 모아보기 · 1개', f'원본 PDF 모아보기 · {count}개')
